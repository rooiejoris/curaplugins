#Name: Skew Double
#Info: make the Zlevel uneven over the x-direction in both directions print. CAUTION: maybe you fan or other parts will collide with the object!
#Depend: GCode
#Type: postprocess
#Param: difference(float:30) max skew difference(mm)
#Param: bedX(float:205) size build platform X(mm)
#Param: zScale(float:100) correction in Z
...
#                       dfactor = (bedX-x)/bedX
#                       dfactor = (0.5*bedX-x)/bedX
                       if x <= (0.5 * realbedX): dfactor = ((0.5 * realbedX)-x)/realbedX
#                       if bedX-x = 0: bedX = bedX + 0.0001
                       if x > (0.5 * realbedX): dfactor = ((0.5 * realbedX)-x)/-realbedX
#not                       if x > 0.5 * bedX: dfactor = 0.5 * bedX/(bedX-x)
# 0.5 * bedX / x - (0.5 * bedX)
...
#uto
                         newE = e + math.fabs(dfactor)*0.3*z/maxZ
#                         newE = e
                         newE = round(newE, 3)
